...
(function() {
  ...
  obstacle = [], gameWidth = canvy[1].width, gameHeight = canvy[1].height,
    bgDrawX1 = 0, bgDrawX2 = 2400, colorCode = [], bg_speed = 7 * sHeight /
    744, first_time = true, bg1X = 0, bg2X = 2400;
...
function welcomeMsg() {
  ...
  if (isPlaying) {
    ctxGame.drawImage(bigS, 292, 57, 97, 27, gameWidth / 2.5, gameHeight / 3,
    ...
    ctxgOvCanv.drawImage(bigS, 394, 57, 97, 25, 60, 45, (sWidth / 1366) *
      gameWidth / 8, (sHeight / 744) * gameHeight / 8);
...
function drawBg() {
  ...
  ctxGame.drawImage(bigS, 292.5, 0, 90.5, 55, bgDrawX1, sHeight * 0.7, 2450,
    sHeight * 0.3); //backg 1
  ctxGame.drawImage(bigS, 292.5, 0, 90.5, 55, bgDrawX2, sHeight * 0.7, 2450,
    sHeight * 0.3); //backg 2*/drawBg
...
function definePipes() {
  var x1 = 800 * sWidth / 1366,
    h1, h2;
  for (i = 0; i < 250; i += 2) {
    h1 = Math.floor(Math.random() * (365 * sHeight / 744));
    h2 = Math.floor(Math.random() * (365 * sHeight / 744));

    while (!((Math.abs(h1 - h2) < (200 * sHeight / 744)) && (h1 < 365 *
          sHeight / 744) && (h2 < 365 * sHeight / 744) && h1 > 40 * sHeight /
        744)) {
      h1 = Math.floor(Math.random() * 365 * sHeight / 744);
      h2 = Math.floor(Math.random() * 365 * sHeight / 744);
...
function drawPipes() {
  ...
  for (i = 0; i < 250; i += 2) {
    ...
    ctxPipes.fillRect(obstacle[i].x_init, ((100 * sHeight / 744) + obstacle[i]
      .height), obstacle[i].width, (sHeight * 0.57) - obstacle[i].height);
    ctxPipes.fill();
...
createFlappy.prototype.draw = function() {
  ...
  ctxFlappy.drawImage(bird, this.srcX, this.srcY, this.width, this.height,
    this.drawX, this.drawY, this.radius * 2, this.radius * 2);
  drawn = true;
...
function checkHit() {
  for (var i = 0; i < 250; i += 2) {
    ...
    if (flappy.drawX + flappy.radius > obstacle[i].x_init && flappy.drawX -
      flappy.radius < obstacle[i].x_init + obstacle[i].width - (10 * sWidth /
        744)) {
      if (flappy.drawY + flappy.radius + (25 * sHeight / 744) <= obstacle[i].height ||
        flappy.drawY >= obstacle[i].height + (35 * sHeight / 744)) {
